    ...
    <div>
        ...
        <div class="mt-6 bg-white rounded shadow overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                <tr
                    v-for="product in products.expenses"
                    :key="product.id"
...
export default {
    metaInfo() {
        return { title: this.products.name };
    },
    layout: Layout,
    components: {
        Icon,
    ...
    methods: {
        ...
        submit() {
            ...
            this.$inertia.put(
                this.route("exptypes.update", this.products.id),
                this.form,
                {
        ...
        destroy() {
            if (confirm("Are you sure you want to delete this type?")) {
                this.$inertia.delete(
                    this.route("exptypes.destroy", this.products.id)
                );
            }
        },
        restore() {
            if (confirm("Are you sure you want to restore this type?")) {
                this.$inertia.put(
                    this.route("exptypes.restore", this.products.id)
                );
            }
        },
        reset() {
