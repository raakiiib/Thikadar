<template>
    <div>
        <h1 class="mb-8 font-bold text-3xl">
            <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('dumping.index')">ব্লক ডাম্পিং</inertia-link>
        ...
        <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
            <form @submit.prevent="submit">
                <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
                    ...
                    <select-input v-model="form.product_id" :error="errors.product_id" class="pr-6 pb-8 w-full lg:w-1/2" label="Service">
                        <option :value="null" />
                        <option v-for="service in services" :key="service.id" :value="service.id">{{ service.name }} ( {{service.dimension}} )</option>             
                    </select-input>

                    <select-input v-model="form.vendor_id" :error="errors.vendor_id" class="pr-6 pb-8 w-full lg:w-1/2" label="Supplier">
                        <option :value="null" />
                    ...
                    <text-input type="number" step="any" v-model="form.size" @input="updateNetAmout" :error="errors.size" class="pr-6 pb-8 w-full lg:w-1/2" label="Size" />
                    
                    <text-input type="number" step="0.01" v-model="form.unitprice" @input="updateNetAmout" :error="errors.unitprice" class="pr-6 pb-8 w-full lg:w-1/2" label="Rate" />

                    <text-input type="number" step="0.01" v-model="form.net_amount" :error="errors.net_amount" class="pr-6 pb-8 w-full lg:w-1/2" label="Total"/>

                    <text-input type="number" step="0.01" v-model="form.paid_amount" @input="calculateDue" :error="errors.paid_amount" class="pr-6 pb-8 w-full lg:w-1/2" label="Paid"/>

                    <text-input type="number" step="-0.01" v-model="form.due_amount" @input="calculateDue" :error="errors.due_amount" class="pr-6 pb-8 w-full lg:w-1/2" label="Due"/>
